...
import FeatureParty from './reusable/FeatureParty.vue';
import TransitionExpand from './TransitionExpand.vue';
import partysData from '../data/party.json';
...
const activeFeature = computed(() =>
  activeFeatureIndex.value !== null ? partys.value[activeFeatureIndex.value] : null
);
  ...
  nextTick(() => {
    ...
    const startAutoChange = () => {
      autoChangeInterval = setInterval(() => {
        const nextIndex = (activeFeatureIndex.value + 1) % partys.value.length;
    ...
    // 用戶互動時暫停自動循環
    const container = document.querySelector('.game-partys');
    if (container) {
...
onMounted(async () => {
  try {
    ...
    partys.value = partysData;
    if (partysData.length > 0) {
...
<template>
  <section ref="sectionRef" class="section game-partys" :class="{ 'is-visible': isVisible }">
    ...
    <div class="container">
      ...
      <div v-else class="partys-content">
        <div class="partys-tabs-container" :class="{ 'animated': tabsAnimated }">
          <div class="partys-tabs">
            <button v-for="(feature, index) in partys" :key="feature.id" class="feature-tab"
        ...
        <TransitionExpand :active-index="activeFeatureIndex">
          <div v-if="activeFeature" :key="activeFeature.id" class="feature-content-wrapper">
            <FeatureParty class="feature-item" :title="activeFeature.title" :description="activeFeature.description"
        ...
        <div class="navigation-dots">
          <button v-for="(_, index) in partys" :key="index" class="dot"
...
.partys-tabs-container.animated {
  opacity: 1;
  transform: translateY(0);
}
...
/* Webkit 滾動條樣式 */
.partys-tabs::-webkit-scrollbar {
  height: 5px;
  background: rgba(255, 255, 255, 0.05);
  border-radius: 5px;
}

.partys-tabs::-webkit-scrollbar-thumb {
  background-color: var(--primary-600);
  border-radius: 5px;
  transition: all 0.3s;
}

.partys-tabs::-webkit-scrollbar-thumb:hover {
